...
HEADERS = [
    "来源文件", "页码", "发票类型", "发票号码", "开票日期", "购买方名称", "购买方税号",
    "销售方名称", "销售方税号", "未税金额", "税额", "价税合计", "币种", "校验状态", "备注",
]
...
def emit(kind: str, payload: dict[str, Any], seq: int, *, correlation: str = "", run: dict[str, Any] | None = None) -> None:
    frame: dict[str, Any] = {"lap": "0.1", "id": f"{AGENT_ID}:{seq}", "producer": AGENT_ID, "seq": seq, "type": kind, "payload": payload}
    if correlation:
...
def fail(message: str, code: str = "LAP-201") -> dict[str, Any]:
    return {"status": "failed", "summary": "发票处理失败。", "error": {"code": code, "message": message, "retryable": code == "LAP-500"}}
...
def vision(image: bytes, media_type: str, detail: str, text_layer: str = "") -> list[dict[str, Any]]:
    key = os.environ.get("DEEPSEEK_API_KEY", "").strip()
    if not key:
        raise RuntimeError("缺少 DEEPSEEK_API_KEY。")
    url = os.environ.get("DEEPSEEK_VISION_BASE_URL", "https://api.deepseek.com/chat/completions")
...
def normalize(raw: dict[str, Any], source: str, page: int) -> tuple[list[Any], list[str]]:
    warnings: list[str] = []
    subtotal, tax, total = (money(raw.get(key)) for key in ("amount_excluding_tax", "tax_amount", "total_amount"))
    ...
    row = [
        source, page, str(raw.get("invoice_type", "")).strip(), number,
        str(raw.get("issue_date", "")).strip(), str(raw.get("buyer_name", "")).strip(),
        str(raw.get("buyer_tax_id", "")).strip(), str(raw.get("seller_name", "")).strip(),
        str(raw.get("seller_tax_id", "")).strip(), float(subtotal) if subtotal is not None else None,
        float(tax) if tax is not None else None, float(total) if total is not None else None,
        str(raw.get("currency", "CNY") or "CNY"), "通过" if not warnings else "需复核",
        "; ".join(warnings + ([str(raw.get("notes", "")).strip()] if raw.get("notes") else [])),
    ]
    return row, warnings
...
def execute(payload: dict[str, Any], run: dict[str, Any], seq: int) -> int:
    ...
    detail = request_input.get("detail", "original") if isinstance(request_input, dict) else "original"
    ...
    try:
        ...
        for path, name in sources:
            for image, media, page, text_layer in image_pages(path, name):
                seq += 1
                emit("run.progress", {"phase": "recognize", "message": f"正在识别 {name} 第 {page} 页"}, seq, run=run)
        ...
        seq += 1
        emit("run.result", {"status": "succeeded", "summary": f"已整理 {len(rows)} 张发票。", "output": {"invoice_count": len(rows), "source_count": len(sources), "workbook": target.name, "warnings": warnings}, "artifacts": [artifact]}, seq, run=run)
        ...
        seq += 1
        emit("run.result", fail("发票识别服务执行失败，请检查模型配置或稍后重试。", "LAP-500"), seq, run=run)
    return seq
...
def main() -> int:
    ...
    for raw in sys.stdin:
        ...
        if kind == "agent.hello":
            seq += 1
            emit("agent.welcome", {"selected_lap": "0.1", "profiles": ["lap-local/0.1"], "agent_id": AGENT_ID, "version": VERSION, "max_concurrency": 1}, seq, correlation=str(message.get("id", "")))
        elif kind == "run.start":
            run = message.get("run") or {}
            seq += 1
            emit("run.accepted", {"capability": "invoice.extract"}, seq, correlation=str(message.get("id", "")), run=run)
