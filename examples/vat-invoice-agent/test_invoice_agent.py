"""Regression tests for the VAT invoice Agent's local data handling."""
...
def test_invalid_invoice_number_requires_review() -> None:
    """Flag an invoice number whose length is outside declared valid forms."""
    row, warnings = normalize(
        {
            "invoice_number": "1234567890123456789",
            "amount_excluding_tax": 159.25,
            "tax_amount": 9.55,
            "total_amount": 168.80,
        },
...
def test_multiple_host_artifacts_are_resolved(tmp_path: Path) -> None:
    """Resolve every digest-verified Host input artifact independently."""
