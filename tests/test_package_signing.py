...
class PackageSigningTests(unittest.TestCase):
    def _sign(
        ...
        package: Path,
        *,
        key_id: str = "com.example.publisher",
    ...
    def test_sidecar_is_excluded_but_every_other_package_byte_is_addressed(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            verification = verify_package_signature(
                ...
                package_sha256=digest,
                trusted_public_keys={
                    "com.example.publisher": public_key_base64url(key)
                },
            ...
            payload = json.loads(
                (package / "lap-signature.json").read_text(encoding="utf-8")
            )
            schema = json.loads(
                (ROOT / "schemas" / "package-signature.schema.json").read_text(
                    encoding="utf-8"
                )
            )
