...
_ENVELOPE_FIELDS = frozenset((
    "lap", "id", "correlation_id", "producer", "seq", "type", "run",
    "idempotency_key", "payload", "extensions",
))
_RUN_FIELDS = frozenset(("tenant_id", "session_id", "run_id", "parent_run_id", "trace_id"))


def _host_frame(message_id: str, sequence: int, message_type: str, payload: dict[str, Any], *,
                run: dict[str, str] | None = None, correlation_id: str | None = None,
...
def _validate_draft_envelope(value: Any, label: str) -> dict[str, Any]:
    ...
    if not isinstance(value.get("seq"), int) or isinstance(value["seq"], bool) or value["seq"] < 0:
    ...
    for field in ("correlation_id", "idempotency_key"):
        if field in value and (not isinstance(value[field], str) or not value[field] or len(value[field]) > 240):
            raise ProbeError("LAP-101", f"{label} has an invalid {field}.")
    if "run" in value:
        run = value["run"]
        ...
        if not required_run.issubset(run) or any(
                not isinstance(run.get(field), str) or not run[field] or len(run[field]) > 240
...
class _LocalExchange:
    ...
    def __init__(self, command: list[str], working_directory: Path, timeout_seconds: float) -> None:
    ...
    def _remaining(self) -> float:
        ...
        if remaining <= 0:
            raise ProbeError("LAP-401", "The Local Agent did not complete the relay probe before its timeout.")
        return remaining

    def send(self, frame: dict[str, Any]) -> None:
        if self.process.stdin is None or self.process.poll() is not None:
            raise ProbeError("LAP-500", "The Local Agent exited before receiving a Host frame.")
        encoded = json.dumps(frame, ensure_ascii=False, separators=(",", ":"))
        if len(encoded.encode("utf-8")) > MAX_FRAME_BYTES:
            raise ProbeError("LAP-500", "The draft probe generated an oversized Host frame.")
        try:
            self.process.stdin.write(encoded + "\n")
            self.process.stdin.flush()
        except OSError as exc:
            raise ProbeError("LAP-500", "The Local Agent stdin is unavailable.") from exc

    def receive(self) -> dict[str, Any]:
        try:
            raw = self._lines.get(timeout=self._remaining())
        except queue.Empty as exc:
            raise ProbeError("LAP-401", "The Local Agent did not emit a relay frame before its timeout.") from exc
        if raw is None:
            raise ProbeError("LAP-500", "The Local Agent exited before completing the relay lifecycle.")
        if not raw.strip():
        ...
        if self._output_bytes > MAX_OUTPUT_BYTES:
            raise ProbeError("LAP-101", "The Local Agent stdout exceeds the relay probe output limit.")
        try:
...
def _require_agent_frame(frame: dict[str, Any], expected_type: str, *, agent_id: str,
                         run: dict[str, str] | None = None) -> None:
...
def _probe_frames(manifest: dict[str, Any], capability: dict[str, Any], probe_input: Any,
                  exchange: _LocalExchange) -> dict[str, Any]:
    ...
    _validate_schema("model-relay-context.schema.json", relay_context, "Relay context", code="LAP-500")
    hello = _host_frame(
        "host-hello", 1, "agent.hello",
        {"supported_lap": ["0.1"], "profiles": ["lap-local/0.1", RELAY_PROFILE], "host": HOST_PRODUCER},
    )
    exchange.send(hello)
    welcome = exchange.receive()
    _require_agent_frame(welcome, "agent.welcome", agent_id=agent_id)
    welcome_payload = welcome["payload"]
    if (welcome.get("correlation_id") != hello["id"]
        ...
        raise ProbeError("LAP-204", "The Local Agent did not negotiate the draft Model Relay Profile.")

    start = _host_frame(
        "host-run-start", 2, "run.start",
        {
            "capability": capability["id"],
            "input": probe_input,
            "context": {"input": probe_input, "extensions": {RELAY_EXTENSION: relay_context}},
        },
        run=run,
    ...
    _validate_schema("model-relay-request.schema.json", request["payload"], "Relay request", code="LAP-101")
    request_payload = request["payload"]
    route = relay_context["routes"][0]
    if request_payload["route"] != route["id"] or request_payload["max_output_tokens"] > route["max_output_tokens"]:
        raise ProbeError("LAP-401", "The Agent relay request exceeds its Host-granted route.")

    response_payload = {
        "status": "succeeded",
        "request_sha256": "d0b8b656f17d8a17789f78bd0c0b59251c43591fa330e17a73e36da5ca0892ce",
        "output": {"text": "Relay probe response."},
        "usage": {"source": "provider", "input_tokens": 16, "cached_tokens": 0, "output_tokens": 4},
    }
    _validate_schema("model-relay-response.schema.json", response_payload, "Relay response", code="LAP-500")
    exchange.send(_host_frame(
        "host-model-response", 3, "model.response", response_payload,
        run=run,
    ...
    while terminal is None:
        ...
        terminal = frame
    _validate_schema("run-result.schema.json", terminal["payload"], "Relay terminal payload", code="LAP-101")
    if terminal["payload"].get("status") != "succeeded":
        raise ProbeError("LAP-500", "The Local Agent did not complete the relay probe successfully.")
    ...
    if any(current <= previous for previous, current in zip(sequences, sequences[1:])):
        raise ProbeError("LAP-101", "The Local Agent relay sequence values are not strictly increasing.")
    if len({frame["id"] for frame in agent_frames}) != len(agent_frames):
        raise ProbeError("LAP-101", "The Local Agent emitted duplicate relay message identifiers.")
    return {
        "valid": True,
        ...
        "relay": {"route": route["id"], "response": "deterministic-host-simulation", "provider_calls": 0},
...
def run_probe(package_path: str, *, capability_id: str | None, input_json: str | None,
    ...
    if not isinstance(profiles, list) or RELAY_PROFILE not in profiles:
        raise ProbeError("LAP-204", "agent.json does not declare the draft Model Relay Profile.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(json.dumps({"valid": False, "error": {
            "code": "LAP-500", "message": f"LAP Model Relay probe failed: {type(exc).__name__}."}},
            ensure_ascii=False, sort_keys=True), file=sys.stderr)
        return 2
